...
class RAGLogger:
    ...
    def _setup_logger(self):
        ...
        # Console handler for errors only - with UTF-8 encoding support
        console_handler = logging.StreamHandler(sys.stdout)
        console_handler.setFormatter(logging.Formatter("%(levelname)s: %(message)s"))
        console_handler.setLevel(logging.ERROR)
        # Set encoding for stdout/stderr to handle UTF-8 properly
    ...
    def milestone(self, message: str, **kwargs):
        ...
        # Normalize Unicode and fix surrogates
        try:
            # Try to fix any surrogate characters
        ...
        try:
            ...
            # Fallback: log ASCII-safe version
            safe_message = message.encode('ascii', errors='replace').decode('ascii')
            self.logger.critical(f"MILESTONE: {safe_message}", **kwargs)
