...
from src.constant import COLLECTION_NAME, DEFAULT_K, RERANK_TOP_K, SIMILARITY_THRESHOLD
from src.rag_builder.connection_manager import get_milvus_client
from src.utils.logging import get_logger
from src.rag_retriever.hybrid_search import hybrid_search
from src.rag_retriever.llm_caller import RAGLLMCaller, get_rag_llm_caller
from src.rag_retriever.model_loader import get_embedding_model, get_reranker_model
...
class DocumentRetriever:
    ...
    def retrieve_and_rerank(self, query: str) -> Tuple[List[Document], List[float]]:
        ...
        try:
            ...
            if embeddings["dense_vecs"].size == 0 or len(embeddings["lexical_weights"]) == 0:
                logger.error("Encoding failed: empty results returned")
                return [], []
